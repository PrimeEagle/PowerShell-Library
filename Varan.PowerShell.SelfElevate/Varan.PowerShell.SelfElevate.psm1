Import-Module Varan.PowerShell.Validation
...
function Open-ElevatedConsole 
{
	...
	{ 
		Write-DisplayDebug "(Now) running as $(("superuser", "admin")[$isWin])."
		return 
	}
	...
	Write-DisplayDebug ("This script, `"$scriptPath`", requires " + ("superuser privileges, ", "admin privileges, ")[$isWin] + ("re-invoking with sudo...", "re-invoking in a new window with elevation...")[$isWin])
	...
	else 
	{
	    ...
	    if ($isWin) 
		{
			$t1 = (Get-Location -PSProvider FileSystem).ProviderPath
			$t2 = $scriptPath
						
			$newStr = '-noexit -c Set-Location "''{0}''"; & "''{1}''"' -f $t1, $t2
						
			Start-Process -Verb RunAs "$psExe" $newStr
	    }
	    else 
		{
